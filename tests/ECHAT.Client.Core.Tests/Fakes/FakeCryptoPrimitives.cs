...
/// NON è crittografia reale: verifica solo round-trip/flow. La correttezza AES-GCM (cifrario,
/// tag, AAD) è coperta dai test JS in <c>tests/js</c>, che esercitano il codice di produzione.
/// </summary>
...
/// <summary><see cref="ISigner"/> fake: SHA-256(data || key). Per i test di delega/flow.</summary>
public sealed class FakeSigner : ISigner
{
    public byte[] Sign(byte[] data, byte[] privateKey) => SHA256.HashData(data.Concat(privateKey).ToArray());
    public bool Verify(byte[] data, byte[] signature, byte[] publicKey) => Sign(data, publicKey).SequenceEqual(signature);
}
